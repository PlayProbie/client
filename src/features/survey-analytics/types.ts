...
/** API 세션 -> 클라이언트 세션 변환 */
export function toSurveySession(api: ApiSurveySession): SurveySession {
  return {
    sessionId: api.session_id,
    surveyName: api.survey_name,
    surveyId: api.survey_id,
    testerId: api.tester_id,
    status: api.status,
    endedAt: api.ended_at,
  };
}

/** API 질문-답변 -> 클라이언트 질문-답변 변환 */
export function toQuestionAnswerExcerpt(
  api: ApiQuestionAnswerExcerpt
): QuestionAnswerExcerpt {
  return {
    qType: api.q_type,
...
/** API 고정 질문 응답 -> 클라이언트 변환 */
export function toFixedQuestionResponse(
  api: ApiFixedQuestionResponse
): FixedQuestionResponse {
  return {
    fixedQId: api.fixed_q_id,
    fixedQuestion: api.fixed_question,
    excerpt: api.excerpt.map(toQuestionAnswerExcerpt),
  };
}

/** API 리스트 아이템 -> 클라이언트 변환 */
export function toSurveyResultListItem(
  api: ApiSurveyResultListItem
): SurveyResultListItem {
  return {
    ...toSurveySession(api),
    firstQuestion: api.first_question,
  };
}

/** API 리스트 -> 클라이언트 변환 */
export function toSurveyResultsList(
  api: ApiSurveyResultsList
): SurveyResultsList {
  return {
    content: api.content.map(toSurveyResultListItem),
...
/** API 요약 -> 클라이언트 변환 */
export function toSurveyResultsSummary(
  api: ApiSurveyResultsSummary
): SurveyResultsSummary {
  return {
    surveyCount: api.survey_count,
    responseCount: api.response_count,
  };
}

/** API 상세 -> 클라이언트 변환 */
export function toSurveyResultDetails(
  api: ApiSurveyResultDetails
): SurveyResultDetails {
  return {
    session: toSurveySession(api.session as ApiSurveySession),
    byFixedQuestion: api.by_fixed_question.map(toFixedQuestionResponse),
  };
}
