...
| **Radix UI / Shadcn UI**  | -          | UI 컴포넌트 (우선 고려) |

### 컴포넌트 추가 규칙

새로운 UI 컴포넌트(Button, Input, Dialog 등)가 필요할 경우, **반드시 Shadcn UI의
구현 패턴과 Radix UI Primitives를 우선적으로 검토하고 적용합니다.**

- 임의의 구현보다 Shadcn UI 표준 구현을 따릅니다.
- 필요 시 `@radix-ui` 패키지를 설치하여 사용합니다.
